...
module.exports = function(filename, data, options, cb){
  ...
  function writeFile(fileWritten){
    var offset = new bigint(4096)
      , stop = false
      , nbRemainingPackets = 0
      , maxPacketSize = new bigint(4096, 0x00010000 - 0x71)
    // callback manager
    ...
    function createPackets(){
      while(nbRemainingPackets<connection.packetConcurrency && offset.lt(fileLength)){
        console.log(`packs remaining: ${nbRemainingPackets}`)
        console.log(`offset: ${offset.toNumber()}/${fileLength.toNumber()}`)
